...
(function (global, factory) {
    ...
    (global ||= self, factory(global.React = {}));
    ...
    let enableScopeAPI = false, // Experimental Create Event Handle API.
     enableCacheElement = false,
     enableTransitionTracing = false, // No known bugs, but needs performance testing
  
     enableLegacyHidden = false, // Enables unstable_avoidThisFallback feature in Fiber
    /*
     * stuff. Intended to enable React core members to more easily debug scheduling
     * issues in DEV builds.
     */
  
     enableDebugTracing = false, // Track which Fiber(s) schedule render work.
    ...
    let isArrayImpl = Array.isArray; // eslint-disable-next-line no-redeclare
    ...
    function typeName(value) {
      {
        ...
        let hasToStringTag = typeof Symbol === "function" && Symbol.toStringTag,
         type = hasToStringTag && value[Symbol.toStringTag] || value.constructor.name || "Object";
        return type;
      }
    } // $FlowFixMe only called in DEV, so void return is not possible.
    ...
    function getWrappedName(outerType, innerType, wrapperName) {
      let displayName = outerType.displayName;
    ...
    function warnIfStringRefCannotBeAutoConverted(config) {
      {
        if (typeof config.ref === "string" && ReactCurrentOwner.current && config.__self && ReactCurrentOwner.current.stateNode !== config.__self) {
          let componentName = getComponentNameFromType(ReactCurrentOwner.current.type);
    ...
    function cloneElement(element, config, children) {
      ...
      let propName, // Original props are copied
       ...
       props = assign({}, element.props), // Reserved names are extracted
  
       key = element.key,
       ref = element.ref, // Self is preserved since the owner is preserved.
  
       self = element._self, // Source is preserved since cloneElement is unlikely to be targeted by a
      /*
       * transpiler, and the original source is probably a better indicator of the
       * true owner.
       */
  
       source = element._source, // Owner will be preserved, unless ref is overridden
    ...
    function mapIntoArray(children, array, escapedPrefix, nameSoFar, callback) {
      ...
      if (invokeCallback) {
        let _child = children,
         mappedChild = callback(_child), // If it's the only child, treat the name as if it was wrapped in an array
        // so that it's consistent if the number of children grows:
  
         childKey = nameSoFar === "" ? SEPARATOR + getElementKey(_child, 0) : nameSoFar;
      ...
      let child,
       nextName,
       subtreeCount = 0, // Count of children found in the current subtree.
      ...
      if (isArray(children)) 
        ...
        if (typeof iteratorFn === "function") {
          ...
          let iterator = iteratorFn.call(iterableChildren),
           step,
           ii = 0;
    ...
    function createContext(defaultValue) {
      ...
      let hasWarnedAboutUsingNestedContextConsumers = false,
       hasWarnedAboutUsingConsumerProvider = false,
       hasWarnedAboutDisplayNameOnConsumer = false;
    ...
    function lazyInitializer(payload) {
      if (payload._status === Uninitialized) {
        let ctor = payload._result,
         thenable = ctor(); // Transition to the next state.
        /*
    ...
    function checkPropTypes(typeSpecs, values, location, componentName, element) {
      {
        ...
        let has = Function.call.bind(hasOwnProperty);
    ...
    function getDeclarationErrorAddendum() {
      if (ReactCurrentOwner.current) {
        let name = getComponentNameFromType(ReactCurrentOwner.current.type);
    ...
    function validatePropTypes(element) {
      {
        ...
        if (propTypes) {
          ...
          let name = getComponentNameFromType(type);
          checkPropTypes(propTypes, element.props, "prop", name, element);
        } else if (type.PropTypes !== undefined && !propTypesMisspellWarningShown) {
          propTypesMisspellWarningShown = true; // Intentionally inside to avoid triggering lazy initializers:
  
          let _name = getComponentNameFromType(type);
    ...
    function siftDown(heap, node, i) {
      ...
      while (index < halfLength) {
        let leftIndex = (index + 1) * 2 - 1,
         ...
         right = heap[rightIndex]; // If the left or right node is smaller, swap with the smaller of those.
    ...
    var getCurrentTime,
     hasPerformanceNow = typeof performance === "object" && typeof performance.now === "function";
    ...
    var maxSigned31BitInt = 1073741823, // Times out immediately
     ...
     IMMEDIATE_PRIORITY_TIMEOUT = -1, // Eventually times out
  
     USER_BLOCKING_PRIORITY_TIMEOUT = 250,
     NORMAL_PRIORITY_TIMEOUT = 5000,
     LOW_PRIORITY_TIMEOUT = 10000, // Never times out
  
     IDLE_PRIORITY_TIMEOUT = maxSigned31BitInt, // Tasks are stored on a min heap
  
     taskQueue = [],
     timerQueue = [], // Incrementing id counter. Used to maintain insertion order.
  
     taskIdCounter = 1, // Pausing the scheduler is useful for debugging.
     currentTask = null,
     currentPriorityLevel = NormalPriority, // This is set while performing work, to prevent re-entrance.
  
     isPerformingWork = false,
     isHostCallbackScheduled = false,
     isHostTimeoutScheduled = false, // Capture local references to native APIs, in case a polyfill overrides them.
     ...
     localSetImmediate = typeof setImmediate !== "undefined" ? setImmediate : null, // IE and Node.js + jsdom
  
     isInputPending = typeof navigator !== "undefined" && navigator.scheduling !== undefined && navigator.scheduling.isInputPending !== undefined ? navigator.scheduling.isInputPending.bind(navigator.scheduling) : null;
    ...
    var isMessageLoopRunning = false,
     ...
     taskTimeoutID = -1, // Scheduler periodically yields in case there is other work on the main
    /*
    ...
    var unstable_requestPaint = requestPaint,
     ...
     Scheduler = /*#__PURE__*/Object.freeze({
      __proto__: null,
      ...
      if (enqueueTaskImpl === null) 
        try {
          ...
          var requireString = ("require" + Math.random()).slice(0, 7),
           nodeRequire = module && module[requireString]; // assuming we're in node, let's try to get node's
